...
db_ctx *init_db_ctx(char *db_path) {
    db_ctx *ctx = malloc(sizeof(db_ctx));
    db_ctx *_ctx = ctx;
...
void deinit_db_ctx(db_ctx *ctx) { CHECK_SQLITE(sqlite3_close(ctx->DB), ctx, "deinit db ctx"); }

db_insert_file_ctx *init_db_insert_file_ctx(db_ctx *ctx) {
    db_insert_file_ctx *stmt_ctx = malloc(sizeof(db_insert_file_ctx));
    stmt_ctx->DB = ctx->DB;
...
void deinit_db_insert_file_ctx(db_insert_file_ctx *ctx) { CHECK_SQLITE(sqlite3_finalize(ctx->stmt), ctx, "db_insert_file_ctx deinit"); }
...
db_insert_chunk_ctx *init_db_insert_chunk_ctx(db_ctx *ctx) {
    db_insert_chunk_ctx *stmt_ctx = malloc(sizeof(db_insert_chunk_ctx));
    stmt_ctx->DB = ctx->DB;
...
void deinit_db_insert_chunk_ctx(db_insert_chunk_ctx *ctx) { CHECK_SQLITE(sqlite3_finalize(ctx->stmt), ctx, "db_insert_chunk_ctx deinit"); }
